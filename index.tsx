...
const PROJECT_DETAILS = {
  title: "DR-KANTreeNet",
  subtitle:
    "Diabetic Retinopathy Classification via Kolmogorov-Arnold Tree Networks",
  abstract:
...
// --- IMPORTANT: GitHub Pages / Vite base path for static assets ---
const BASE_URL = import.meta.env.BASE_URL;

// --- Model Architecture Explanation Blocks ---
const ARCH_COMPONENTS = [
  {
    title: "Input & Preprocessing",
    icon: "fa-image",
    points: [
      "Fundus image input (retinal scan).",
      "Resizing/normalization for consistent downstream feature extraction.",
    ],
  },
  {
    title: "ResNet50 Feature Pyramid",
    icon: "fa-layer-group",
    points: [
      "ResNet50 (features_only) extracts multi-scale features (C2–C5).",
      "Captures local lesion textures and mid/high-level semantics.",
    ],
  },
  {
    title: "Enhanced Lesion Attention",
    icon: "fa-crosshairs",
    points: [
      "Uses channel/lesion attention to emphasize lesion-prone regions.",
      "Guides the network toward microaneurysms/exudates/hemorrhages.",
    ],
  },
  {
    title: "KAN-based Adaptation (KANLayer / KANDAM)",
    icon: "fa-bezier-curve",
    points: [
      "KAN layers learn edge-wise activation functions (more expressive than MLP).",
      "Discriminative attention (KANDAM) refines fine-grained patterns.",
    ],
  },
  {
    title: "ViT Branch (Global Context)",
    icon: "fa-chess-board",
    points: [
      "ViT-S / ViT-L provide global context via patch attention.",
      "Complements CNN locality with long-range dependencies.",
    ],
  },
  {
    title: "VesselTreeNet Branch",
    icon: "fa-project-diagram",
    points: [
      "Vessel mask extraction emphasizes vascular structure.",
      "Helps identify DR progression via vessel abnormalities.",
    ],
  },
  {
    title: "Fusion & Heads",
    icon: "fa-code-branch",
    points: [
      "Concatenate/fuse ResNet+ViT+vessel features.",
      "Top head outputs class logits; optional bottom head adds auxiliary signals (e.g., GCN).",
    ],
  },
];
      ...
      <div className="flex items-center justify-between h-16">
        ...
        <div className="hidden md:block">
          <div className="ml-10 flex items-baseline space-x-4">
            {["Motivation", "Approach", "Architecture", "Implementation", "Results", "Discussion"].map(
              (item) => (
                <a
                  key={item}
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="text-center mb-20">
        ...
        <p className="mt-6 text-slate-400 max-w-2xl mx-auto text-lg">
          Bridging the gap between silent pathology and automated detection through
          Computer Vision.
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2">
          ...
          <div className="p-8 md:p-12 flex flex-col justify-center">
            ...
            <p className="text-slate-300 leading-relaxed mb-6">
              Diabetic Retinopathy disrupts the retina's healthy vascular structure.
              The disease manifests as distinct visual anomalies—<strong>hard exudates</strong>{" "}
              (lipid deposits) and hemorrhages—which create pixel-level contrasts against
              the retina.
            </p>
            <p className="text-slate-400 text-sm mb-6 border-l-2 border-cyan-500 pl-4 italic">
              "It is this exact visual distinction that enables us to employ Computer
              Vision. By training KANTreeNet to recognize these specific textures and
              color gradients, we can automate screening with superhuman consistency."
            </p>
            ...
            <div className="space-y-4">
              <div className="flex gap-4 items-center">
                ...
                <span className="text-slate-300 text-sm">
                  Healthy: Clear vessels, uniform macula.
                </span>
              </div>
              <div className="flex gap-4 items-center">
                ...
                <span className="text-slate-300 text-sm">
                  Affected: Yellow lipid exudates, dark hemorrhages.
                </span>
              </div>
            </div>
          </div>
      ...
      <div className="text-center mb-10">
        ...
        <p className="text-slate-400 text-sm mt-2">
          Solving the limitations of traditional Deep Learning
        </p>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-12">
          ...
          <p className="text-slate-400 text-lg max-w-2xl">
            A step-by-step breakdown of how DR-KANTreeNet processes a retinal scan,
            from raw input to final diagnosis.
          </p>
        </div>
                ...
                <div className="absolute bottom-0 w-full bg-black/80 backdrop-blur-md p-6 border-t border-slate-700">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-slate-500 text-xs mt-2 italic border-l-2 border-slate-700 pl-3">{step.details}</p>
                    </div>
                  </div>
                </div>
...
// --- NEW: Architecture Section (Diagram + Explanation) ---
const Architecture = () => (
  <section id="architecture" className="py-24 bg-slate-900">
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">

      <div className="text-center mb-12">
        <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">Model Architecture</h2>
        <div className="w-20 h-1 bg-cyan-500 mx-auto rounded-full"></div>
        <p className="mt-6 text-slate-400 max-w-3xl mx-auto text-lg">
          The enhanced DR-KANTreeNet integrates CNN multi-scale features, transformer global context, vessel-structure cues,
          and KAN-based adaptive transformations to produce interpretable and robust DR severity predictions.
        </p>
      </div>
      ...
      <div className="bg-slate-950 rounded-2xl border border-slate-700 overflow-hidden shadow-2xl">
        <div className="p-4 md:p-6 border-b border-slate-800 flex items-center justify-between">
          <div className="text-slate-300 font-semibold">
            ...
            Enhanced DR-KANTreeNet Architecture Diagram
          </div>
          <a
        ...
        <div className="p-3 md:p-6 bg-black">
          <img
            ...
            alt="Enhanced DR-KANTreeNet Architecture"
      ...
      <div className="mt-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {ARCH_COMPONENTS.map((c, idx) => (
          <div
            key={idx}
            className="p-6 rounded-2xl bg-slate-800 border border-slate-700 hover:border-cyan-500/40 transition-colors"
          >
          ...
          <div className="col-span-2 p-6 bg-slate-800 rounded-xl border border-slate-700">
            ...
            <div className="flex gap-4 flex-wrap">
              {PROJECT_DETAILS.stack.map((tech) => (
                <span
                  key={tech}
                  className="px-3 py-1 bg-slate-700 rounded-full text-xs text-cyan-300 border border-slate-600"
                >
                  {tech}
                </span>
              ))}
        ...
        <div className="lg:col-span-3 bg-slate-900 rounded-xl border border-slate-800 p-6">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {PROJECT_DETAILS.results.comparison.map((row: any, i: number) => {
                  const isBest = (metric: string) => row.bestMetrics && row.bestMetrics.includes(metric);
                  return (
                    <tr
                      key={i}
                    ...
                    >
                      ...
                      <td className={`py-4 px-4 text-right ${isBest("auc") ? "text-cyan-400 font-bold text-lg" : row.highlight ? "text-slate-300" : "text-slate-400"}`}>
                        {row.auc}
                      </td>
                      <td className={`py-4 px-4 text-right ${isBest("acc") ? "text-cyan-400 font-bold text-lg" : row.highlight ? "text-slate-300" : "text-slate-400"}`}>
                        {row.acc}
                      </td>
                      <td className={`py-4 px-4 text-right ${isBest("f1") ? "text-cyan-400 font-bold text-lg" : row.highlight ? "text-slate-300" : "text-slate-400"}`}>
                        {row.f1}
                      </td>
                      <td className={`py-4 px-4 text-right ${isBest("kappa") ? "text-cyan-400 font-bold text-lg" : row.highlight ? "text-slate-300" : "text-slate-400"}`}>
                        {row.kappa}
                      </td>
                    </tr>
                  );
  ...
  <footer id="discussion" className="bg-slate-950 border-t border-slate-800 py-12">
    <div className="max-w-7xl mx-auto px-4 text-center">
      ...
      <p className="max-w-3xl mx-auto text-slate-400 mb-8 leading-relaxed">
        The comparative analysis demonstrates that DR-KANTreeNet achieves robust performance, matching or exceeding strong baselines from recent years like TransMIL and MIL-ViT in terms of accuracy and F1 score. However, it still falls slightly short of the latest state-of-the-art models like MIL-ViTa in AUC and Kappa metrics.
        <br /><br />
        Crucially, while MIL-ViTa optimizes purely for predictive performance, DR-KANTreeNet prioritizes <strong>clinical interpretability</strong> via its differentiable decision tree head. Future work will aim to close this performance gap by incorporating more advanced attention mechanisms while retaining the transparency that clinicians require.
      </p>
              ...
              <div key={i} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
                <div
                  className={`max-w-[85%] rounded-2xl px-4 py-2 text-sm ${
                    m.role === "user" ? "bg-cyan-600 text-white rounded-br-none" : "bg-slate-800 text-slate-200 rounded-bl-none"
                  }`}
                >
                  {m.text}
                </div>
